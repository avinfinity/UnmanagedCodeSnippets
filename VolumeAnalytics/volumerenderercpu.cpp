...
namespace imt 
{
	...
	namespace volume 
	{
		...
		static inline float D(int x, int y, int z, int nVoxels[3], unsigned short* density) 
		{
			x = Clamp(x, 0, nVoxels[0] - 1);
			y = Clamp(y, 0, nVoxels[1] - 1);
			z = Clamp(z, 0, nVoxels[2] - 1);
		...
		static inline float D(int x, int y, int z, int nVoxels[3], unsigned short* density, float* transferFunction)
		{
			x = Clamp(x, 0, nVoxels[0] - 1);
			y = Clamp(y, 0, nVoxels[1] - 1);
			z = Clamp(z, 0, nVoxels[2] - 1);

			return transferFunction[ 4 * density[z * nVoxels[0] * nVoxels[1] + y * nVoxels[0] + x]];
		}
		...
		static inline float Density(float3 Pobj, float3 pMin, float3 pMax, unsigned short* volumeData , float* transferFunction, int nVoxels[3])
		{
			if (!Inside(Pobj, pMin, pMax))
				return 0;
			// Compute voxel coordinates and offsets for _Pobj_
			...
			// Trilinearly interpolate density values to compute local density
			float d00 = Lerp(dx, D(vx, vy, vz, nVoxels, volumeData, transferFunction), D(vx + 1, vy, vz, nVoxels, volumeData, transferFunction));
			float d10 = Lerp(dx, D(vx, vy + 1, vz, nVoxels, volumeData, transferFunction), D(vx + 1, vy + 1, vz, nVoxels, volumeData, transferFunction));
			float d01 = Lerp(dx, D(vx, vy, vz + 1, nVoxels, volumeData, transferFunction), D(vx + 1, vy, vz + 1, nVoxels, volumeData, transferFunction));
			float d11 = Lerp(dx, D(vx, vy + 1, vz + 1, nVoxels, volumeData, transferFunction), D(vx + 1, vy + 1, vz + 1, nVoxels, volumeData, transferFunction));


			unsigned short d0 = static_cast<unsigned short>( Lerp(dy, d00, d10));
			unsigned short d1 = static_cast<unsigned short>( Lerp(dy, d01, d11));
		...
		static float transmittance(float3 p0, float3 p1, float3 pMin, float3 pMax, float sigma_t, unsigned short* density,
			float* transferFunction ,int nVoxels[3]) {
			float rayT0, rayT1;
			Ray ray;
			ray.origin = p1;
			ray.dir = p0 - p1;
			...
			float3 dirStep = ray.dir * stepT;
			while (t < rayT1) {
				tau += stepDist * sigma_t * Density(pos, pMin, pMax, density, transferFunction , nVoxels);
				pos = pos + dirStep;
				t += stepT;
			}
		...
		static float raymarch(unsigned short* density , float* transferFunction, int nVoxels[3], const Ray& ray) {
			float rayT0, rayT1;
			float3 pMin(0, 0, 0), pMax(nVoxels[0], nVoxels[1], nVoxels[2]);
			float3 lightPos(-1.f, 4.f, 1.5f);
			...
			float sigma_a = 10;        // Absorption coefficient
			float sigma_s = 10;        // Scattering coefficient
			float stepDist = 1.0;//0.25f;//0.025f;   // Ray step amount
			float lightIntensity = 40; // Light source intensity
			...
			float3 dirStep = ray.dir * stepT;
			while (t < rayT1) {
				float d = Density(pos, pMin, pMax, density, transferFunction, nVoxels);
				...
				// direct lighting
				float Li = lightIntensity / distanceSquared(lightPos, pos) *
					transmittance(lightPos, pos, pMin, pMax, sigma_a + sigma_s, density , transferFunction, nVoxels);
				L += stepDist * atten * d * sigma_s * (Li + Le);
		...
		VolumeRendererCPU::VolumeRendererCPU(unsigned short* volumeData, int w, int h, int d) :
			mVolumeData(volumeData) , mWidth(w) , mHeight(h), mDepth(d)
		{
			mHistogram.resize(std::numeric_limits<unsigned short>::max());

			mTransferFunction = 0;
		}

		void VolumeRendererCPU::render( TrackBallCamera* camera )
		{
			int viewPortW = camera->viewportWidth();
			int viewPortH = camera->viewportHeight();

			std::vector < Eigen::Vector3f > origins(viewPortW * viewPortH), rays(viewPortW * viewPortH);

			cv::Mat renderedImage( viewPortH , viewPortW , CV_8UC1 );

			int offset = 0;

			int volumeDim[]{ mWidth , mHeight , mDepth };

			auto buffer =  mTransferFunction->GetBuffer();

			for (int yy = 0; yy < viewPortH; yy++)
				for (int xx = 0; xx < viewPortW; xx++, offset++)
				{
					...
					//image[offset] = raymarch(density, nVoxels, ray);
					int gVal = static_cast<unsigned char>(raymarch(mVolumeData, buffer , volumeDim, ray) * 255.0);
					renderedImage.data[offset] = gVal;// 

					if (offset % 64 * 48 == 0)
						std::cout << offset / (64 * 48.0) << std::endl;
					

					//renderedImage.data[offset] = 

				}

			std::cout << "offset : " << offset << " " << 640 * 480 << std::endl;


			cv::namedWindow("VolumeRendering", 0);
			cv::imshow("VolumeRendering", renderedImage);
			cv::waitKey();
		...
		void VolumeRendererCPU::setTransferFunction(TransferFunction* transferFunction)
		{
