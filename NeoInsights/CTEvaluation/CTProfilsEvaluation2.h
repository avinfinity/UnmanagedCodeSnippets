#ifndef __IMT_CTPROFILSEVALUATION_H__
#define __IMT_CTPROFILSEVALUATION_H__
...
#include "NeoInsights/CTEvaluation/vmclasses.h"			// IZM-Vektor und Matrix
#include "NeoInsights/CTEvaluation/lusolver.h"				// IZM-Vektor und Matrix
#include "NeoInsights/CTEvaluation/svdsolver.h"			// IZM-Vektor und Matrix
...
//CCTProfilsEvaluation - Evaluation of a set of gray value profiles, especially determination of gray value profiles

//CCTProfilsEvaluation maintains a number of gray - value profiles of the same length \c length, whose gray values are successively in a memory section at equidistant intervals(\c voxelStep voxel).
/// Internally is addressed and calculated in integer indexes, the voxel length is obtained by multiplication by \ c voxelStep. All gray values are converted into the type Float32 for the internal calculation in the constructor.

//When initializing the instance, several filter cores are calculated for the given filter parameter \ c sigma to calculate the different derivatives(canny etc.) from the gray value profile.
/// If you change \ c sigma, these must be created again, so the function \ c SetSigma () must be used. For the other parameters threshold value, search range, etc. are also
/// to use the appropriate set functions. It allocates different buffer areas for interim results in order to avoid time-consuming allocations during the calculation.
/// CCTProfilsEvaluation uses the optimized vector functions from the Intel Performance Primitives library for the calculations performed in profile mode.
/// It was linked to the non-multithreading-capable libraries, because the multithreading over the distribution of the measuring points to several instances of CCTProfilsEvaluation by the superordinate class
...
protected:
	...
	//Presetting of the integration range depends on \ c sigma, the default is 3.5 * sigma after each side of the integration reference point
	double	rangeFactor;
	...
	//Pointer to field with extracted gray value profiles converted to(float) in continuous order(profile i has address "profile + i * length")
	float*	profile;		


	/// Länge eines Grauwertprofilfelds

	//Length of a gray value profile field
	long	length;	
	...
	//Previously calculated filter core for canny filtering(first derivative)
	...
	//Previously calculated filter core for the determination of the second lead
	...
	//Buffer for filter core for invoices outside the index points
...
public:
	...
	/// If, at the pre-calculated position \c x of the gray value profile, a second gradient is in the other direction in the vicinity, this can falsify the determination of the gradient maximum if it is included in the filtering.
	/// This function is used to search for interfering neighboring gray-value transitions and to adjust the filter width dynSigma so that they do not influence the result. The chosen factors and limits are heuristic!
	...
	/// It is searched in both directions from the zero point of the profile (== position of the default point), the first gradient maximum that satisfies the parameters \ c threshold and \ searchRange is returned.
	/// where \ c x the profile parameter is stored (reference length to the zero point \ c zeroIdx).
	/// In the \ c tempConvProfile is the profile of the second derivative.
