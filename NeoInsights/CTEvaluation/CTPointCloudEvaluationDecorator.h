...
public:
					...
					)
	{
		...
		double const searchRangeVoxel = std::max(origSearchRangeVoxel, 3.0);
		double const origSearchRangeAirSideVoxel = searchRangeAirSide / voxelSizeMax;
		double const searchRangeAirSideVoxel = std::max(origSearchRangeAirSideVoxel, 3.0);
		...
		double timeSpentInExtractingProfiles = (cv::getTickCount() - initT) / cv::getTickFrequency();

		std::cout << "time spent in extracting profiles : " << timeSpentInExtractingProfiles << std::endl;
		...
		if (useAutoParams || useBeamHardeningCorrection)
		{
			...
			if (success)
				std::cout << "AutoParam successful!" << std::endl; //log->LogDebug("AutoParam successful!");
			else
				std::cout << "AutoParam failed!" << std::endl; //log->LogDebug("AutoParam failed!");
		...
		std::cout << "Starting to measure " << count << " points (Quality: " << usingQuality ? "true)..." : "false)...";
		//log->LogDebug(msg.str());
		msg.str("");
		...
		{	
			if (!evaluation.getResult(i, results[i], normals[i], &qualities[i]))//(measurementProgress != nullptr && measurementProgress->IsCanceled()) || 
			{
