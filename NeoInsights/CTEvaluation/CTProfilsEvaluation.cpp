...
*/
bool	CCTProfilsMeasure::SearchAroundZero( double& x, long iProfil, float fSearchRange, float fSearchRangeNeg, double staticTest , 
	                                         double airPointsThresh , bool dynControl , int sign )
{
	...
	double x_vw = 0.0, x_rw = 0.0;						// Treffer der Vor- und Rückwärtssuche
	...
	if(	p.ptValid[iProfil] != true || p.results[iProfil] > 1e6)
	{
		...
		long i_vw = p.convProfileZeroIndex, i_rw = p.convProfileZeroIndex;	// Index der Vor- und Rückwärtssuche
...
bool	CCTProfilsMeasure::SearchAroundZero2(double& x, long iProfil, float fSearchRange, float fSearchRangeNeg, double staticTest ,
	double airPointsThresh , bool dynControl , int sign )
{
	bool result = true;
	assert(p.threshold > 0.0);
...
long CCTProfilsMeasure::SearchGlobally(long iProfil, long dir, CDPVector& list, double airPointsThresh)
{
	...
	long i_vw = p.convProfileZeroIndex - p.searchRangeNeg, i_rw = p.convProfileZeroIndex + p.searchRange - 1;	// Index der Vor- und Rückwärtssuche
	double x_vw = 0.0, x_rw = 0.0, val_vw = 0.0, val_rw = 0.0;						// Treffer der Vor- und Rückwärtssuche
	// Loop mit gleichteitiger Vor- und Rückwärtssuceh
...
bool CCTProfilsMeasure::SearchAroundZeroMixedMat(double& x,long iProfil,double globalGradThreshold, std::vector<int> &materialThresholds,std::vector<int> &materialMasks)
{
	...
	p.convProfileZeroIndex = p.FoldSecDerOpt(iProfil,p.tempConvProfile);
	long nPeaks =0;	// Anzahl gefundener Gradientenmaxima
	long i_vw = p.convProfileZeroIndex - p.searchRangeNeg;	// Index der Vor- und Rückwärtssuche
	double x_vw = 0.0, val_vw = 0.0;						// Treffer der Vor- und Rückwärtssuche
