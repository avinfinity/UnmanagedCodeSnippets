...
vtkStandardNewMacro(vtkModifiedCutter);
vtkCxxSetObjectMacro(vtkModifiedCutter,CutFunction,vtkImplicitFunction);
vtkCxxSetObjectMacro(vtkModifiedCutter,Locator,vtkIncrementalPointLocator)

//----------------------------------------------------------------------------
// Construct with user-specified implicit function; initial value of 0.0; and
// generating cut scalars turned off.
vtkModifiedCutter::vtkModifiedCutter(vtkImplicitFunction *cf)
{
...
//----------------------------------------------------------------------------
vtkModifiedCutter::~vtkModifiedCutter()
{
...
vtkMTimeType vtkModifiedCutter::GetMTime()
{
...
//----------------------------------------------------------------------------
void vtkModifiedCutter::StructuredPointsCutter(vtkDataSet *dataSetInput,
...
//----------------------------------------------------------------------------
void vtkModifiedCutter::StructuredGridCutter(vtkDataSet *dataSetInput,
                                     vtkPolyData *thisOutput)
{
...
//----------------------------------------------------------------------------
void vtkModifiedCutter::RectilinearGridCutter(vtkDataSet *dataSetInput,
                                      vtkPolyData *thisOutput)
{
...
//
int vtkModifiedCutter::RequestData(
  vtkInformation *request,
  vtkInformationVector **inputVector,
  vtkInformationVector *outputVector)
{
  ...
  if ((input->GetDataObjectType() == VTK_STRUCTURED_POINTS ||
	...
	std::cout << " data set cutter " << std::endl;
...
//----------------------------------------------------------------------------
void vtkModifiedCutter::GetCellTypeDimensions(unsigned char* cellTypeDimensions)
{
...
//----------------------------------------------------------------------------
void vtkModifiedCutter::DataSetCutter(vtkDataSet *input, vtkPolyData *output)
{
  vtkIdType cellId, i;
  int iter;
  vtkPoints *cellPts;
  ...
  if ( this->SortBy == VTK_SORT_BY_CELL )
    {
		...
		std::cout << " sort by cell " << std::endl;
    ...
    //vtkModifiedCutter::GetCellTypeDimensions(cellTypeDimensions);
    int dimensionality;
    // We skip 0d cells (points), because they cannot be cut (generate no data).
    //for (dimensionality = 2; dimensionality <= 2; ++dimensionality)
	dimensionality = 2;
	{
	 //	std::cout << "dimensionality :" << dimensionality << std::endl;
      ...
      // and process each cell.
      //
      for ( cellId=0; cellId < numCells / 20 && !abortExecute; cellId++ )
        {
...
//----------------------------------------------------------------------------
void vtkModifiedCutter::UnstructuredGridCutter(vtkDataSet *input, vtkPolyData *output)
{
  vtkIdType i;
  int iter;
  vtkDoubleArray *cellScalars;
...
void vtkModifiedCutter::CreateDefaultLocator()
{
  if ( this->Locator == NULL )
    {
		this->Locator = vtkMergePoints::New();//vtkIncrementalOctreePointLocator::New(); //
...
//----------------------------------------------------------------------------
int vtkModifiedCutter::RequestUpdateExtent(
  vtkInformation *,
  vtkInformationVector **inputVector,
...
//----------------------------------------------------------------------------
int vtkModifiedCutter::FillInputPortInformation(int, vtkInformation *info)
{
...
//----------------------------------------------------------------------------
void vtkModifiedCutter::PrintSelf(ostream& os, vtkIndent indent)
{
