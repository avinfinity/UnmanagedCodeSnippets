...
{
	//std::cout << "Last compiled: " << __DATE__ << " " << __TIME__ << std::endl;
...
//std::vector<uint> ZeissBackend::computeMultiResolutionFactors( std::tr1::shared_ptr<Image::Image3DDataUI16> pVolume,
//                                                               Math::Vector3ui minSize,
//                                                               bool bDimensionsOrVoxels,
//                                                               bool bUseMultipleOfTwo,
//                                                               uint nResolutions,
//                                                               bool bLargeVolumeAdjustment)
//{
//   std::vector<uint> vecResult;
//   uint nOldWidth = pVolume->size()(0);
//   uint nOldHeight = pVolume->size()(1);
//   uint nOldDepth = pVolume->size()(2);

//   // work around for high resolution miss detections
//   // check for high res volumes
//   bool bHighResVolume = false;
//   bool bUltraHighResVolume = false;
//   if ((size_t)768*768*768 < nOldWidth*nOldHeight*nOldDepth)    bHighResVolume = true;
//   if ((size_t)1536*1536*1536 < nOldWidth*nOldHeight*nOldDepth) bUltraHighResVolume = true;

//   //adjust start values to ensure proper results even with high resolutions 
//   uint nStartValue = 1;
//   if(bHighResVolume && bLargeVolumeAdjustment)
//      nStartValue = 2;
//   if(bUltraHighResVolume && bLargeVolumeAdjustment)
//      nStartValue = 3;

//   vecResult.push_back(nStartValue);

//   if (nResolutions==1) return vecResult;   

//   if (bUseMultipleOfTwo) nStartValue *= 2;
//   else nStartValue += 1;

//   //uint nIncrement = 1;
//   //if (bUseMultipleOfTwo) nIncrement = 2;

//   uint nNewWidth; uint nNewHeight; uint nNewDepth;
//   uint nMaxReductionFactor = std::min(std::min(nOldWidth, nOldHeight), nOldDepth);
//   //for (uint i = nStartValue; i < nMaxReductionFactor; i += nIncrement) {
//   for (uint i = nStartValue; i < nMaxReductionFactor;) {
//      nNewWidth = floor(static_cast<float>(nOldWidth) / i);
//      nNewHeight = floor(static_cast<float>(nOldHeight) / i);
//      nNewDepth = floor(static_cast<float>(nOldDepth) / i);

//      //check minimal volume dimensions
//      if(bDimensionsOrVoxels) {
//         if (nNewWidth > minSize(0) && nNewHeight > minSize(1) && nNewDepth > minSize(2)) {
//            vecResult.push_back(i);
//         }
//      } else {
//         if (minSize(0) * minSize(1) * minSize(2) < nNewWidth * nNewHeight * nNewDepth) {
//            vecResult.push_back(i);
//         }
//      }
//      if(nResolutions <= vecResult.size()) break;

//      //new resolution step for next iteration
//      if (bUseMultipleOfTwo) i *= 2;
//      else i += 1;

//   }

//   for (uint n = vecResult.size(); n < nResolutions; n++) {
//      vecResult.push_back(vecResult.at(vecResult.size() - 1));
//   }
//   return vecResult;
//}
...
std::tr1::shared_ptr<Image::DataCollector<uint16> > ZeissBackend::createVolumeCollector(Math::Vector3ui& volumeSize) {
	...
	//TODO: 768.0, 1536.0? empirical values
	if (768.0 * 768.0 * 768.0 < ((double) volumeSize(0)) * volumeSize(1) * volumeSize(2) / scalingFactor)    bHighResVolume = true;
	if (1536.0 * 1536.0 * 1536.0 < ((double) volumeSize(0)) * volumeSize(1) * volumeSize(2) / scalingFactor) bUltraHighResVolume = true;
   ...
   //CHANGED: 19.10 HiF/MeS
	//if (scaling(0) == 1 && scaling(1) == 1 && scaling(2) == 1 /* for testing purposes */ && m_numberSeparationThreads > 0) {
	//	return pMultiplexer;
	//} else {
...
void ZeissBackend::segmentMaterials() {
	// - create instance of HistogramProbabilityAnalyser, fit normal distributions on the histgram and store it to m_pProbabilities
	// - create instance of ProbabilitySegmentation and segment the volume according to Probabilities and the Metrialindex
	...
	//std::cout << "Segment materials: " << (boost::posix_time::microsec_clock::local_time() - segmentmaterials_start).total_seconds() << std::endl;
}
...
void ZeissBackend::calclulateRegionsOfInterest() { //TODO: Rechtschreibfehler: sollte calculate heissen!
	...
	//std::cout << "Separation took: " << (end - start).total_milliseconds() << "ms" << std::endl;
...
void ZeissBackend::separateAndSave(const std::string& basename) {
	...
	double fToleranceFactor = 4.0; //maybe 8? old value 2 //TODO
...
std::string ZeissBackend::description() const {
	...
	if (m_pCurrentStep) {return m_pCurrentStep->description();}
	else {return std::string("Zeiss Volume Separation");}
}
...
double ZeissBackend::doBenchmark(char* path, size_t size, bool reset){
	...
	//std::string strapppath = ("v:\\01_software\\fbpartviewer\\fbpartviewer\\volumes\\bench.xml");
	...
	if(boost::filesystem::exists(m_strPathBenchmark) && boost::filesystem::extension(m_strPathBenchmark) == ".xml"){   
		...
		BOOST_FOREACH(boost::property_tree::ptree::value_type &pTreeValue, ptree.get_child("Benchmark.Measurements")){ // changed the variable name of the iterator from v to pTreeValue
			if(m_bGPU == pTreeValue.second.get<bool>("GPU") && m_nSegmentationRadius == pTreeValue.second.get<size_t>("FilterRadius"))
			{
				...
				//std::cout << nTimeNormalDistribution << std::endl;
				//std::cout << nTimeFilterRadius << std::endl;
				//std::cout << nTimePostprocessing << std::endl;
				...
				//float fCPUPerformanceValue = pTreeValue.second.get<float>("CPUPerformanceValue");
				//size_t nThreads = static_cast<int>(nThreads*fCPUPerformanceValue);
				//std::cout << "current threads: " << nThreads << std::endl;
				//if(nThreads<1)
				//   nThreads = 1;
		...
		if(nCounter == 0){
			...
			//std::cout << "First region: " << m_pRegions->data()[0].second << std::endl;
			//std::cout << "Second region: " << m_pRegions->data()[1].second << std::endl;
