...
bool CCTPointCloudEvaluation::AutoParam(long step, bool bCorrProfileMat, bool bCorrProfileAir, bool bAutoSigma, bool bAutoGradientThreshold)
{
	...
	if(bAutoGradientThreshold)
	{
		...
		else _Log->LogDebug("PointCloud - AutoParam: No threshold found");
	}
	else
	{
		std::stringstream msg;
		...
		do
		{
			...
			if(resultSigma) 
			{
				...
				if(bAutoGradientThreshold)
				{
					...
					else _Log->LogDebug("PointCloud - Recalculation of Gradient threshold failed");
				}
			}
			else _Log->LogDebug("PointCloud - AutoParam: No sigma found");
		}
			...
			if(!abort)
			{
				...
				for(int i = 0; i < pSlice[iSlice].nProfils; i++)
				{
					...
					if(nMaterial==1)
					{
						...
						{
							double xx;
							if(pSlice[iSlice].ptValid[i]) xx = pSlice[iSlice].results[i];
							else success = pM.SearchAroundZero(xx,i,float(fSearchRange),float(fSearchRangeNeg),-1,-1,false);
							if(pSlice[iSlice].ptValid[i])
							{
								pSlice[iSlice].ProfileCorrection(xx,i,profileCorrMat?corrPolyMat:0,profileCorrAir?corrPolyAir:0);
								if(!profileIsCorrected) profileIsCorrected = true;
							}
						}
