...
#include "iostream"
	...
	const char *getOSName(OSVERSIONINFO *pOSVI)
	{
		...
		std::cout << " major and minor versions : " << pOSVI->dwMajorVersion << " " << pOSVI->dwMinorVersion << std::endl;

		if ((pOSVI->dwMajorVersion == 6) && (pOSVI->dwMinorVersion == 2))
			return (const char *)"Windows 10";
                     ...
                     char **argv)
{
	...
	std::string oclDir = std::string(ROOT_DIR) + "/OpenCL";

    size_t program_length;
	const char* source_path = shrFindFilePath(ocl_source_filename, argv[0]);//

	std::cout <<  oclDir.c_str() <<" " <<ocl_source_filename<< std::endl;
