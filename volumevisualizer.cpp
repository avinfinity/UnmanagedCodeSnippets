...
VolumeVisualizer::VolumeVisualizer(Q3DScatter *scatter)
    : m_graph(scatter),
      m_volumeItem(0),
      ...
      m_sliceLabelZ(0)
{
	...
	qDebug() << " loading volume  " << mVolumeDataPath << endl;
    ...
    if (!isOpenGLES()) 
	{
		...
		//if (volume)
		//{
			m_inputdata = new QVector<uchar>(256 * 256 * 110);

			char *rawData = data.data();

			for ( int dd = 0; dd < 110; dd++ )
			{
				qDebug() << ( int )( (unsigned char *)rawData )[dd * 256 * 256 + 128 * 256 + 128] << endl;
			}

			memcpy(m_inputdata->data(), data.data(), 256 * 256 * 110 );
		//}
		...
		qDebug() << " low detail size : " << lowDetailSize << endl;
		...
		m_volumeItem->setTextureWidth(256);
		m_volumeItem->setTextureHeight(256);
		m_volumeItem->setTextureDepth(110);
		m_volumeItem->setTextureFormat(QImage::Format_Indexed8);
		m_volumeItem->setTextureData(new QVector<uchar>(*m_inputdata));
		...
		for (int i = 0; i < colorTableSize - 2; i++)
		{
			...
			{
				m_colorTable1[i] = qRgba( i , 0 , 0, 255);
			}
		}

        for (int i = 0; i < 0; i++)//colorTableSize - 2 
		{
            if (i < magmaColorsMax) 
			{
		...
		//m_colorTable1[0] = qRgba(0, 0, 0, 0);

		qDebug() << " color value at 0 index : " << m_colorTable1[0] << endl;
...
else 
{
    ...
    QObject::connect(m_graph, &QAbstract3DGraph::currentFpsChanged, this,
                     &VolumeVisualizer::handleFpsChange);
    QObject::connect(&m_timer, &QTimer::timeout, this,
                     &VolumeVisualizer::handleTimeout);
...
void VolumeVisualizer::setFpsLabel(QLabel *fpsLabel)
{
    m_fpsLabel = fpsLabel;
}

void VolumeVisualizer::setMediumDetailRB(QRadioButton *button)
{
    m_mediumDetailRB = button;
}

void VolumeVisualizer::setHighDetailRB(QRadioButton *button)
{
    m_highDetailRB = button;
}

void VolumeVisualizer::setSliceLabels(QLabel *xLabel, QLabel *yLabel, QLabel *zLabel)
{
...
void VolumeVisualizer::setAlphaMultiplierLabel(QLabel *label)
{
    m_alphaMultiplierLabel = label;
}

void VolumeVisualizer::sliceX(int enabled)
{
    m_slicingX = enabled;
    handleSlicingChanges();
}

void VolumeVisualizer::sliceY(int enabled)
{
    m_slicingY = enabled;
    handleSlicingChanges();
}

void VolumeVisualizer::sliceZ(int enabled)
{
    m_slicingZ = enabled;
    handleSlicingChanges();
}

void VolumeVisualizer::adjustSliceX(int value)
{
    if (m_volumeItem) {
...
void VolumeVisualizer::adjustSliceY(int value)
{
    if (m_volumeItem) {
...
void VolumeVisualizer::adjustSliceZ(int value)
{
    if (m_volumeItem) {
...
void VolumeVisualizer::handleFpsChange(qreal fps)
{
...
void VolumeVisualizer::handleTimeout()
{
...
void VolumeVisualizer::toggleLowDetail(bool enabled)
{
...
void VolumeVisualizer::toggleMediumDetail(bool enabled)
{
    if (enabled && m_volumeItem) {
...
void VolumeVisualizer::toggleHighDetail(bool enabled)
{
    if (enabled && m_volumeItem) {
...
void VolumeVisualizer::setFpsMeasurement(bool enabled)
{
...
void VolumeVisualizer::setSliceSliders(QSlider *sliderX, QSlider *sliderY, QSlider *sliderZ)
{
...
void VolumeVisualizer::changeColorTable(int enabled)
{
    if (m_volumeItem) {
        //if (enabled)
...
void VolumeVisualizer::setPreserveOpacity(bool enabled)
{
...
void VolumeVisualizer::setTransparentGround(bool enabled)
{
    if (m_volumeItem) {
        //! [12]
...
void VolumeVisualizer::setUseHighDefShader(bool enabled)
{
    if (m_volumeItem) {
        //! [13]
...
void VolumeVisualizer::adjustAlphaMultiplier(int value)
{
    if (m_volumeItem) {
        float mult;
        if (value > 100)
...
void VolumeVisualizer::toggleAreaAll(bool enabled)
{
    if (enabled) {
...
void VolumeVisualizer::toggleAreaMine(bool enabled)
{
    if (enabled) {
...
void VolumeVisualizer::toggleAreaMountain(bool enabled)
{
    if (enabled) {
...
void VolumeVisualizer::toggleVolumeForInputData( bool enabled )
{

}

void VolumeVisualizer::setDrawSliceFrames(int enabled)
{
    if (m_volumeItem)
        m_volumeItem->setDrawSliceFrames(enabled);
}

void VolumeVisualizer::initHeightMap(QString fileName, QVector<uchar> &layerData)
{
    QImage heightImage(fileName);

	qDebug() << " height image file name " << fileName << endl;
...
int VolumeVisualizer::createVolume(int textureSize, int startIndex, int count,
...
int VolumeVisualizer::excavateMineShaft(int textureSize, int startIndex, int count,
                                          QVector<uchar> *textureData)
{
    int endIndex = startIndex + count;
...
void VolumeVisualizer::excavateMineBlock(int textureSize, int dataIndex, int size,
                                           QVector<uchar> *textureData)
{
...
void VolumeVisualizer::handleSlicingChanges()
{
    if (m_volumeItem) {
...
void VolumeVisualizer::initMineShaftArray()
{
